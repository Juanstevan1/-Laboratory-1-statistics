---
title: "Gabriel Alberto Agudelo Torres_Samuel Acosta Aristizábal"
author: "Samuel Acosta, David Grisales, Juan Garcia"
...
Estudiantes <- read_excel(ruta_excel)
```
...
informacion <- data.frame(
  ...
  Tipo = sapply(Estudiantes, class),
  Caracteristica = sapply(Estudiantes, function(x) {
    if (is.factor(x)) {
      "Cualitativa Nominal"
...
# Calcula resúmenes por área
summary_por_area <- Estudiantes %>%
  summarise(mean_Biología = mean(Puntaje_Biología),
            sd_Biología = sd(Puntaje_Biología),
            mean_Lenguaje = mean(Puntaje_Lenguaje),
            sd_Lenguaje = sd(Puntaje_Lenguaje),
            mean_Matemáticas = mean(Puntaje_Matemáticas),
            sd_Matemáticas = sd(Puntaje_Matemáticas),
            mean_Humanidades = mean(Puntaje_Humanidades),
            sd_Humanidades = sd(Puntaje_Humanidades))
summary_por_area


# Resúmenes por Categoría
categoria_comparison <- caso_ideal %>%
  group_by(Categoría) %>%
  summarise(mean_Biología = mean(Puntaje_Biología),
            mean_Lenguaje = mean(Puntaje_Lenguaje),
            mean_Matemáticas = mean(Puntaje_Matemáticas),
            mean_Humanidades = mean(Puntaje_Humanidades))
categoria_comparison


# Resúmenes por Sede
sede_comparison <- Estudiantes %>%
  group_by(Sede) %>%
  summarise(mean_Biología = mean(Puntaje_Biología),
            mean_Lenguaje = mean(Puntaje_Lenguaje),
            mean_Matemáticas = mean(Puntaje_Matemáticas),
...
# Resúmenes por Clasificación de Bachillerato
bachillerato_comparison <- Estudiantes %>%
  group_by(Clasificación_Bachillerato) %>%
  summarise(mean_Biología = mean(Puntaje_Biología),
            mean_Lenguaje = mean(Puntaje_Lenguaje),
            mean_Matemáticas = mean(Puntaje_Matemáticas),
            mean_Humanidades = mean(Puntaje_Humanidades))
bachillerato_comparison

# Resúmenes por Clasificación de Primaria
primaria_comparison <- Estudiantes %>%
  group_by(Clasificación_Primaria) %>%
  summarise(mean_Biología = mean(Puntaje_Biología),
            mean_Lenguaje = mean(Puntaje_Lenguaje),
            mean_Matemáticas = mean(Puntaje_Matemáticas),
            mean_Humanidades = mean(Puntaje_Humanidades))
...
ggplot(categoria_comparison, aes(x = Categoría, y = mean_Biología)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Biología por Categoría",
       x = "Categoría",
       y = "Puntaje de Biología")

ggplot(categoria_comparison, aes(x = Categoría, y = mean_Lenguaje)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Lenguaje por Categoría",
       x = "Categoría",
       y = "Puntaje de Lenguaje")

ggplot(categoria_comparison, aes(x = Categoría, y = mean_Matemáticas)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Matemáticas por Categoría",
       x = "Categoría",
       y = "Puntaje de Matemáticas")

ggplot(categoria_comparison, aes(x = Categoría, y = mean_Humanidades)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Humanidades por Categoría",
       x = "Categoría",
       y = "Puntaje de Humanidades")


# Gráficos de barras por Sede
ggplot(sede_comparison, aes(x = Sede, y = mean_Biología)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Biología por sede",
       x = "Sede",
       y = "Puntaje de Biología")

ggplot(sede_comparison, aes(x = Sede, y = mean_Lenguaje)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Lenguaje por sede",
       x = "Sede",
       y = "Puntaje de Lenguaje")

ggplot(sede_comparison, aes(x = Sede, y = mean_Matemáticas)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Matemáticas por sede",
       x = "Sede",
       y = "Puntaje de Matemáticas")

ggplot(sede_comparison, aes(x = Sede, y = mean_Humanidades)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Humanidades por sede",
       x = "Sede",
       y = "Puntaje de Humanidades")

# Gráficos de barras por Clasificación de Bachillerato
ggplot(bachillerato_comparison, aes(x = Clasificación_Bachillerato, y = mean_Biología)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Biología por Clasificación",
       x = "Clasificación",
       y = "Puntaje de Biología")

ggplot(bachillerato_comparison, aes(x = Clasificación_Bachillerato, y = mean_Lenguaje)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Lenguaje por Clasificación",
       x = "Clasificación",
       y = "Puntaje de Lenguaje")

ggplot(bachillerato_comparison, aes(x = Clasificación_Bachillerato, y = mean_Matemáticas)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Matemáticas por Clasificación",
       x = "Clasificación",
       y = "Puntaje de Matemáticas")

ggplot(bachillerato_comparison, aes(x = Clasificación_Bachillerato, y = mean_Humanidades)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Humanidades por Clasificación",
       x = "Clasificación",
       y = "Puntaje de Humanidades")

# Gráficos de barras por Clasificación de Primaria
ggplot(primaria_comparison, aes(x = Clasificación_Primaria, y = mean_Biología)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Biología por Clasificación Primaria",
       x = "Clasificación Primaria",
       y = "Puntaje de Biología")

ggplot(primaria_comparison, aes(x = Clasificación_Primaria, y = mean_Lenguaje)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Lenguaje por Clasificación Primaria",
       x = "Clasificación Primaria",
       y = "Puntaje de Lenguaje")

ggplot(primaria_comparison, aes(x = Clasificación_Primaria, y = mean_Matemáticas)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Matemáticas por Clasificación Primaria",
       x = "Clasificación Primaria",
       y = "Puntaje de Matemáticas")

ggplot(primaria_comparison, aes(x = Clasificación_Primaria, y = mean_Humanidades)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Comparación de puntajes de Humanidades por Clasificación Primaria",
       x = "Clasificación Primaria",
       y = "Puntaje de Humanidades")


```
El análisis de los datos revela facetas fundamentales acerca del rendimiento estudiantil y la distribución de variables clave. 

###Comparación Analítica por Sede:

Al realizar un cotejo entre las distintas sedes, se observa que la sede urbana ciudad lidera en Biología y Matemáticas, en tanto que la sede rural sobresale en Lenguaje y Humanidades. Estas disparidades apuntan a la influencia que pueden tener el entorno y los recursos en el desempeño académico, destacando la imperante necesidad de ajustar las estrategias educativas según las peculiaridades inherentes a cada localidad.

###Divergencias de Relevancia entre Tipos de Sede:

Las discrepancias manifiestas en las puntuaciones por sede sugieren distinciones de relevancia. Las sedes urbana municipio y urbana ciudad exhiben puntuaciones generalmente más elevadas, insinuando posiblemente la disponibilidad de recursos y condiciones de aprendizaje más propicias en entornos urbanos. Esta disparidad se acentúa especialmente en Lenguaje y Humanidades. No obstante, se impone la necesidad de llevar a cabo un análisis estadístico más exhaustivo para corroborar la significancia de estas diferencias.

###Pertinencia de la Clasificación para Bachillerato:

El escrutinio de la clasificación de rendimiento en bachillerato evidencia que, en términos generales, los estudiantes clasificados como "REND_SUPERIOR" obtienen las puntuaciones más elevadas en todas las áreas. Este respaldo sustancial corrobora la pertinencia de la clasificación actual, al existir una correlación manifiesta entre el desempeño en la secundaria y el rendimiento en las distintas áreas de conocimiento.